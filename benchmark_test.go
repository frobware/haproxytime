...
func BenchmarkParseDurationMultiUnitMode(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := haproxytime.ParseDuration("24d20h31m23s647ms", haproxytime.Millisecond, haproxytime.ParseModeMultiUnit, haproxytime.NoRangeChecking)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkParseDurationSingleUnitMode(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := haproxytime.ParseDuration("2147483647ms", haproxytime.Millisecond, haproxytime.ParseModeSingleUnit, haproxytime.NoRangeChecking)
		if err != nil {
			b.Fatal(err)
		}
	}
